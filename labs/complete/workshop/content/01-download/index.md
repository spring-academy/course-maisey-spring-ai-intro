---
title: Download the Support Assistant Sample Code
---
...
```terminal:execute
command: zip -qr sample-code.zip sample-app spring-releases-mcp-server -x "*/target/*"
session: 1
description: Zip both projects into sample-code.zip
```

Check that the archive was created.

```terminal:execute
command: ls -lh ~/sample-code.zip
session: 1
...
Click the action below to download `sample-code.zip` to your own machine.

```files:download-file
path: sample-code.zip
```
